...
## 配置step2路径
step1_train_model_path = os.path.join(root, 'runs/detect/train2/weights/best.pt')
...
## 配置step5路径
step5_teacher_model_path = "/content/drive/MyDrive/AI project/runs/detect/train19/weights/best.pt"
step5_student_model_path = "/content/drive/MyDrive/AINEWPROJECT/yolov11_prune_distillation_v2/runs/detect/finetune2/weights/best.pt"
step5_output_model_path = os.path.join(root, "runs/detect/student")

def step1_train():
    model = YOLO(pretrained_model_path)
    model.train(data=yaml_path, device="0", imgsz=640, epochs=50, batch=16, workers=8, save_period=1)  # train the model


def step2_constraint_train():
    model = YOLO(step1_train_model_path)
    model.train(data=yaml_path, device="0", imgsz=640, epochs=50, batch=16, amp=True, workers=8, save_period=1,
...
def step4_finetune():
    ...
    model.train(data=yaml_path, device="0", imgsz=640, epochs=150, batch=16, workers=8,
...
def step5_distillation():
    ...
    model_s.train(data=yaml_path, Distillation=model_t.model, loss_type='mgd', layers=layers, amp=True, imgsz=640,
                  epochs=150,
                  batch=16, device=0, workers=8, lr0=0.001, name=step5_output_model_path)
